      ...
      try {
        ...
        await Promise.all(
          data.map(async (service) => {
            try {
              const price = await calculateCoinPrice(Number(service.rate), service.service)
              prices[service.service] = price
            } catch (err) {
              console.error(`Error calculating price for service ${service.service}:`, err)
              prices[service.service] = Number(service.rate) * 1.5 // Fallback: provider rate * 1.5
            }
          })
        )
              ...
              {filteredServices.map((service) => {
                ...
                return (
                  <option key={service.service} value={service.service}>
                    {service.name} ({service.type}) - {priceDisplay}/1000
                  </option>
                )
              })}
            </select>
